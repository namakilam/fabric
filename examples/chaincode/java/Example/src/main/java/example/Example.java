...
public class Example extends ChaincodeBase {
	...
	public Response init(ChaincodeStub chaincodeStub) {
		chaincodeStub.putState("hello", "world".getBytes());
		return new Response(Response.Status.SUCCESS, "Initialization O.K", null);
	}
	...
	public Response invoke(ChaincodeStub chaincodeStub) {
		switch (chaincodeStub.getFunction()) {
			case "invoke":
				return this.Invoke(chaincodeStub);
			case "query":
				return this.Query(chaincodeStub);
		}

		return new Response(Response.Status.SUCCESS, "Incorrect Function Call", null);
	}

	private Response Query(ChaincodeStub chaincodeStub) {
		try {
			...
			return newSuccessResponse(chaincodeStub.getState(args.get(0)));
		} catch (Exception e) {
			return newErrorResponse(e);
		}
	}

	private Response Invoke(ChaincodeStub chaincodeStub) {
		try {
			List<String> args = chaincodeStub.getParameters();

			if (args.size() != 2) {
				return new Response(Response.Status.SUCCESS, "Incorrect Number Of Arguments", null);
			}

			String key = args.get(0);
			String value = args.get(1);

			chaincodeStub.putState(key, value.getBytes());
			return newSuccessResponse(String.format("Successfully Added Value : %s to Key : %s In the Ledger.", value, key));
		} catch (Exception e) {
			return newErrorResponse(e);
		}
	}
